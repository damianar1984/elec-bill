...
import { German } from 'flatpickr/dist/l10n/de.js'; // Deutsche Lokalisierung
...
export function ZaehlerstandErfassen() {
  ...
  useEffect(() => {
    ...
    if (datepickerRef.current) {
      flatpickr(datepickerRef.current, {
        ...
        locale: German, // Deutsche Lokalisierung
        maxDate: 'today', // Keine Zukunftsdaten
        defaultDate: datum,
  ...
  async function holeLetzterZaehlerstand() {
    try {
      ...
      if (!data || typeof data !== 'object') {
        throw new Error('Ungültiges Antwortformat von read-last-data-sheet');
      }
  ...
  async function handleZaehlerstandErfassen() {
    try {
      ...
      const response = await fetch(
        'https://kfosgimkfydlbranmqvv.supabase.co/functions/v1/write-meter-reading',
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${anonKey}`,
          ...
          body: JSON.stringify({
            ...
            datum: datum, // Bereits im Format DD.MM.YYYY
            sendInvoice,
          }),
        }
      );
      ...
      setSpeicherStatus(result.message);
  ...
  return (
    <div className="rounded-xl bg-white p-6 shadow-lg transition-shadow hover:shadow-xl">
      ...
      <div className="mb-4 flex items-center">
        <span className="mr-3 text-sm font-semibold text-gray-700">Rechnung verschicken</span>
        <div
      ...
      {speicherStatus && (
        <div
          className={`mt-4 rounded-lg p-4 text-sm ${
            speicherStatus.includes('Fehler')
