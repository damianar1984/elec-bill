...
export function App() {
  ...
  async function holeMieter() {
    try {
      ...
      const { data, error } = await supabase.functions.invoke<MieterAntwort>('read-tenant-sheet')
      
      if (error) {
        console.error('Supabase Funktionsfehler:', error)
        throw new Error(`Fehler beim Aufrufen der Funktion: ${error.message}`)
      }
      
      if (data?.mieter) {
        setMieter(data.mieter)
      } else {
        console.error('Keine Mieterdaten in der Antwort:', data)
        throw new Error('Keine Mieterdaten verfügbar')
      }
    } catch (error) {
      console.error('Detaillierter Fehler beim Laden der Mieter:', error)
      setFehler('Die Mieterdaten konnten nicht geladen werden. Bitte überprüfen Sie Ihre Internetverbindung und versuchen Sie es erneut.')
    } finally {
          ...
          <div className="mb-4">
            ...
            {ladevorgang ? (
              ...
              >
                ...
                {mieter.map((mieter, index) => (
                  <option key={index} value={mieter}>
                    {mieter}
                  </option>
                ))}
              </select>
